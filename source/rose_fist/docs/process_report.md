...
## 迭代评估 11：用户要求“握住玫瑰”的遮挡关系
- 发现/问题：用户不接受“整朵玫瑰贴在手上”，需要“枝条在拳头里、花朵在拳头外”。
- 解决：将玫瑰按颜色启发式分层（stem vs flower），绘制顺序 stem → 用拳头区域遮挡 stem → flower。
- 结果：视觉上更像“握住”。

## 迭代评估 12：方向约束升级
- 发现/问题：用户要求（1）玫瑰主方向与拳头方向 90° 垂直；（2）花朵在大拇指那一侧。
- 解决：
  - 拳头方向用 wrist→middle_mcp 估计。
  - 将“花朵方向（stem锚点→花朵质心）”旋转到 `拳头方向+90°`。
  - 若花朵落在非拇指侧，则整体再翻转 180°。
- 结果：满足“垂直+拇指侧”约束。
